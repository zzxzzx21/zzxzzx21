...
import com.example.bank_manager3.bean.BankCardBean;
...
//            ResultSet resultSet = statement.executeQuery("SELECT * FROM finance." + table + ";");
                ResultSet resultSet = statement.executeQuery("SELECT * FROM finance.bank_card order by bc_number;");
                //打印结果
                System.out.println("SELECT FROM : BankCard");
                ...
                try {
                    ...
                    while (resultSet.next()) {
                        ...
                        for (int j = 1; j <= count; j++) {
                            bankCardBean.setInfo(j, resultSet.getString(j));
                        }
